...
function queryParamsToQueryString(queryParams: { [key: string]: boolean | number | string | string[] }) {
   if (!queryParams) {
      return '';
   }
...
export const api = (() => {
   ...
   async function request<T>(path: string, method: Method, config: InferredConfig<T> = {}) {
      ...
      const { queryParams, body, headers: headersOverride, responseType = 'json' } = config;
